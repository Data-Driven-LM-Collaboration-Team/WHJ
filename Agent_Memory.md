...
现有基于记忆的LLM智能体方法主要存储原始轨迹，这些轨迹往往包含大量冗余信息和噪声，导致智能体难以从中提取高层次、可复用的行为模式，而这些模式恰恰是实现泛化的关键。

#### 挑战

如何在原始经验与策略提升之间建立有效桥梁，让智能体不仅能记住过去的经历，更能从经历中提炼出可复用的技能，并使这些技能随着策略的演进而同步演化。这需要解决技能自动发现、技能库高效检索、以及技能与策略协同演化三大难题。

#### 方法

论文提出**SkillRL框架**，通过**自动技能发现**与**递归演化**机制连接原始经验与策略提升。核心包括三大创新组件：①**基于经验的蒸馏机制**——将成功和失败的智能体轨迹蒸馏为紧凑、可复用的技能，构建分层技能库SkillBank（层次化的技能存储结构，区分**通用技能**和**任务特定技能**）；②**自适应检索策略**——基于语义相似度匹配，根据当前任务描述动态获取通用启发式规则和任务特定启发式规则；③**递归演化机制**——使技能库与智能体策略在强化学习过程中协同演化，每个验证周期后，系统分析失败轨迹，识别技能缺口，自动生成新技能或优化现有技能，确保知识库持续适应策略提升和新挑战。

<img width="1914" height="811" alt="image" src="https://github.com/user-attachments/assets/4885da13-fb12-4fcc-9fca-f09d4bcf74d8" />
